#!/usr/bin/env python3
"""
Filter YouTube Comments with Midm 2.0 Base Instruct via vLLM
Reads combined_data.jsonl, scores comments based on criteria using Midm,
...
def main():
    parser = argparse.ArgumentParser(description="Filter comments using Midm 2.0 Base Instruct via vLLM")
    parser.add_argument("--input", "-i", default="comment_results/combined_data.jsonl", help="Input JSONL file")
    parser.add_argument("--output", "-o", default="comment_results/filtered_comments_midm.jsonl", help="Output JSONL file")
    parser.add_argument("--host", default="http://localhost:8000/v1", help="vLLM API Base URL")
    parser.add_argument("--model", default="K-intelligence/Midm-2.0-Base-Instruct", help="Model name")
            ...
            while retry_count < max_retries:
                try:
                    ...
                    try:
                        ...
                        print(f"  ??Failed to parse TSV response from Midm: {e}")
